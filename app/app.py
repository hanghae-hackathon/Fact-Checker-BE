...
app = Flask(__name__)
CORS(app, resources={r"/*":{"origins":"https://app.ekwak.com"}})
env = dotenv_values()
...
def execute_script():
    ...
    if not requested_url:
        return jsonify({"error": "Wrong requested_url"}), 400
    else:
        ...
        if serp_result:
            ...
            for organic in serp_result['organic_results']:
                snippets += f"snippet:{organic.get('snippet')} link:{organic.get('link')}"
